...
#sub() and gsub() function in R are replacement functions, 
#which replaces the occurrence of a substring with other substring
...
trunc(-125.2395)
# truncate function in R for a vector
trunc(c(1.234,2.342,-4.562,5.671,12.345,-14.567))

##Substring Function in R
#The substring function in R can be used either to extract parts of character strings, 
#or to change the values of parts of character strings
#SYNTAX
...
#paste0 function in R simply concatenates the vector with space separator.
...
#strsplit function in R is used to split the string into substrings with the specified delimiter. 
#So that the resultant substrings are separated by a delimiter.
...
#With Function in R evaluate the R expression in an environment constructed locally by the data. 
#It doesn't create a copy of the data.
#Within Function in R evaluates the R expression in an environment constructed 
#locally and it creates a copy of data.
...
#In with function, while executing the expression in an environment. 
#It doesn't save the copy of data frame. 
#so the addition of two columns will not be reflected 
#in the output. 
# with function in R
df$c<-with(df, c <- a + b)
df
...
#by() Function in R
#by() function in R applies a function to specified subsets of a data frame.
#First parameter of by() function, takes up the data and second parameter is by which the 
...
raj<-aggregate(iris[,1:4],by=list(iris$Species),mean,na.ram=T)
...
#Reshape function in R transforms the data from wide to long and also transforms back 
#the data from long to wide. Reshape in R - reshape(), is one of the efficient function to 
#transform the data.
# create data frame
...
###grep() and grepl()
##R provides such functions (grep and grepl) that match character patterns in specified vector.  
#While both of these functions find patterns, they return different output types based on those patterns.  
#Specifically, grep returns numeric values that correspond to the indexed locations of the 
#patterns and grepl returns a logical vector in which "TRUE" represents a pattern match.
head(CO2)
tail(CO2)
...
library(dplyr)
CO2_dplyr<-tbl_df(CO2) 
#converting CO2 into a local data frame 
...
select_dplyr_columns<-CO2_dplyr %>% 
  select(grep("T", colnames(CO2_dplyr)))
select_dplyr_columns
dont_select_dplyr_column<-CO2_dplyr %>% 
  select(-grep("T", colnames(CO2_dplyr)))
...
#then COMBINE the results. A more detailed discussion of this strategy is found in Hadley Wickham's Journal of 
...
#i.e. For when you have several data structures (e.g. vectors, lists) and 
#you want to apply a function to the 1st elements of each, 
...
#mapply sums up all the first elements(1+1+1) ,sums up all the second elements(2+2+2) and so on
# mapply function in R
mapply(rep,1:4,1:4)#it repeats the first element once ,second element twice and so on


##tapply function in R: For when you want to apply a function to subsets of a vector and 
...
##rapply function in R: rapply function in R is nothing but recursive apply, 
#as the name suggests it is used to apply a function to all elements of a list recursively
# rapply function in R
x=list(1,2,3,4)
rapply(x,function(x){x^2},class=c("numeric"))
# rapply function in R for subsists
...
#***Note: Correlation in R cannot be calculated if values has NA. So use = "complete.obs"  
#*neglects NAs while calculating correlation coefficient in R
...
#Rank Function in R
#rank() function in R returns the ranks of the values in a vector. 
#rank function in R also handles Ties and missing values in several ways
