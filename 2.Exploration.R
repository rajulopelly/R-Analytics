...
which (is.na(airquality$Solar.R)) #Which one is NA?
summary(airquality) #which variable has NA's
describe(airquality)
library(psych)

complete.cases(airquality) 
#complete.cases() is a logical vector with the value TRUE for rows 
...
#Complete.cases allows partial selection by including only certain columns of the dataframe####
#sum(row.has.na)
...
summary(airquality)  
#Verification of NA's after replacing by zero
...
library(ggplot2)
data("diamonds")
#using diamond set for this package
mydata<-diamonds
str(mydata)
ncol(mydata)
names(mydata)
